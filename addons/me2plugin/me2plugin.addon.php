...
if ($called_position == 'before_display_content' && Context::getResponseMethod() == 'HTML')
{
	...
	{
		$output = preg_replace('/<!--BeforeDocument\('.$oDocument->get('document_srl').',[0-4]+\)-->/', "$0 $buttonHtml", $output);
	}
	else
	{
		$output = preg_replace('/<!--AfterDocument\('.$oDocument->get('document_srl').',[0-4]+\)-->/', "$buttonHtml $0", $output);
	}
	...
	// Apply button html
	$output = preg_replace('/<!--AfterDocument\('.$oDocument->get('document_srl').',[0-4]+\)-->/', "$commentHtml $0", $output);
}
